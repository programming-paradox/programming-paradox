...
def FirePayload(filePath, encryptPass):
    ...
    pyAesCrypt.encryptFile(filePath, (filePath+'.aes'),
                           encryptPass, bufferSize)  # encrypt
...
def GetTargets():
    ...
    delayTime = int(
        input("Time in seconds to wait between checking the account?\n>"))
    filePath = input("File to encrypt if switch fires?\n>")
    encryptPass = input("Password to encrypt file?\n>")
    targetTime = (
        time.time() + (int(input("How many minutes to run before firing?\n>"))*60))
    c.Hide_output = True
    c.Store_object = True
    CheckKey(c, delayTime, filePath, encryptPass, targetTime)
